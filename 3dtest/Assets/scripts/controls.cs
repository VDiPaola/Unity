...
public class controls : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            //removes old probe if it exists
            if (GameObject.FindGameObjectWithTag("probe")) { Destroy(GameObject.FindGameObjectWithTag("probe")); }
            //sends out probe
            GameObject probeClone = Instantiate(probe, probe.transform.position, probe.transform.rotation);

            probeClone.GetComponent<Rigidbody>().velocity = (probeClone.GetComponent<Transform>().right ) * 10;
            probeClone.gameObject.tag = "probe";

            probeClone.SetActive(true);
        }
        if (Input.GetKeyDown("z") && GameObject.FindGameObjectWithTag("probe"))
        {
            //attaches you to probe
